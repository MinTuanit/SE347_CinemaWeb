...
export class InvoicesService {
  ...
  async create(payload: any) {
    const { data, error } = await this.supabase.from('invoices').insert(payload).select();
    if (error) throw error;
    return data;
  }

  async update(id: number, payload: any) {
    const { data, error } = await this.supabase.from('invoices').update(payload).eq('id', id).select();
    if (error) throw error;
    return data;
  }
